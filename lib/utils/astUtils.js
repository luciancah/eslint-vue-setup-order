import { DEFAULT_SECTION_ORDER, INTERFACE_LIST, DECLARE_LIST, LIFECYCLE_LIST } from "../constants.js";
...
export function unwrapTypeCast(node) {
  const isTSNode = (n) => n.type === "TSAsExpression" || n.type === "TSTypeAssertion";
...
function getVariableDeclarationSection(node) {
  ...
  if (declaration && declaration.init) {
    ...
    if (initNode.type === "FunctionExpression" || 
...
export function getCallExpressionSection(calleeName) {
  ...
  if (INTERFACE_LIST.includes(calleeName)) return "defineOthers";
