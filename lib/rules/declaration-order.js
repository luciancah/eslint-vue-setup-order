...
/**
 * @param {object} params - { node, index, sourceCode, sectionOrder }
 * @returns {object} { node, index, section, sortIndex, text }
 * @description
 * 각 노드별로 섹션과 정렬 인덱스를 생성합니다.
 * 사용자가 전달한 sectionOrder 배열에 해당 섹션이 존재하면 그 인덱스를 사용하고,
 * 존재하지 않으면 sectionOrder.length (즉, 하단에 배치)로 지정합니다.
 */
function createNodeWithSection({ node, index, sourceCode, sectionOrder }) {
